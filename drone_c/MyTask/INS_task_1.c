...
extern TIM_HandleTypeDef htim2;//测试一下定时器取时间
void INS_task(void const *pvParameters)
{
    ...
    while (1)
    {
	watch=__HAL_TIM_GET_COUNTER(&htim2);	//测试一下	__HAL_TIM_GET_COUNTER没跑起来
        ...
        //GYRO零漂补偿和方位设定rotate and zero drift 
        imu_cali_slove(INS_gyro, INS_accel, INS_mag, &bmi088_real_data, &ist8310_real_data);
				
        //加速度计低通滤波  accel low-pass filter
        ...
        accel_fliter_3[2] = accel_fliter_2[2] * fliter_num[0] + accel_fliter_1[2] * fliter_num[1] + INS_accel[2] * fliter_num[2];			
				...
				//加速度计和陀螺仪数据融合算法，去除了磁力计（磁力计好大的零漂x）
				AHRS_update(INS_quat, timing_time, INS_gyro, accel_fliter_3, INS_mag);//除去磁力计
        get_angle(INS_quat, INS_angle + INS_YAW_ADDRESS_OFFSET, INS_angle + INS_PITCH_ADDRESS_OFFSET, INS_angle + INS_ROLL_ADDRESS_OFFSET);
				
				////单位转换
				for(int i=0;i<3;i++)
				{
				IMU_DATA.angle_in_degree[i]=INS_angle[i]*57.296;//正常弧度转角度	
				IMU_DATA.angular_speed[i]=bmi088_real_data.gyro[i]*60;
					//确认是角速度，但单位有问题,差个系数,然后我对照数据手册算不出单位？！
					//考虑到只是给云台速度环PID当actual_value，相差个系数不影响
				}
				
				 ////yaw数据可能会超过360,处理一下方便过零检测
    if (IMU_DATA.angle_in_degree[0] - IMU_DATA.YawTotalAngleLast > 180.0f)
    {
        IMU_DATA.YawRoundCount--;
    }
    else if (IMU_DATA.angle_in_degree[0] - IMU_DATA.YawTotalAngleLast < -180.0f)
    {
        IMU_DATA.YawRoundCount++;
    }
    IMU_DATA.YawTotalAngle = 360.0f * IMU_DATA.YawRoundCount + IMU_DATA.angle_in_degree[0];
    IMU_DATA.YawTotalAngleLast = IMU_DATA.angle_in_degree[0];		
...
void AHRS_update(fp32 quat[4], fp32 time, fp32 gyro[3], fp32 accel[3], fp32 mag[3])
{
    MahonyAHRSupdate(quat, gyro[0], gyro[1], gyro[2], accel[0], accel[1], accel[2], 0.0f, 0.0f, 0.0f);//最后三个0代表不融合磁力计数据
}
...
//    gyro_cali_offset[0] = local_cali_t.offset[0];
//    gyro_cali_offset[1] = local_cali_t.offset[1];
//    gyro_cali_offset[2] = local_cali_t.offset[2];
//    gyro_offset[0] = local_cali_t.scale[0];
//    gyro_offset[1] = local_cali_t.scale[1];
//    gyro_offset[2] = local_cali_t.scale[2];
//	//	以下等价于例程中的  gyro_cali_fun(local_cali_t->scale, local_cali_t->offset, &count_time);
//        static uint16_t count_time = 0;
//        gyro_offset[0] = gyro_cali_offset[0];
//        gyro_offset[1] = gyro_cali_offset[1];
//        gyro_offset[2] = gyro_cali_offset[2];				
//	
//	while(count_time<GYRO_CALIBRATE_TIME)
//					{
...
//					}
//	
//				local_cali_t.offset[0] = gyro_offset[0];
//        local_cali_t.offset[1] = gyro_offset[1];
//        local_cali_t.offset[2] = gyro_offset[2];
...
////        else
////        {
////            RC_unable(); //disable the remote control to make robot no move
//////            imu_start_buzzer();
////        }
//}
