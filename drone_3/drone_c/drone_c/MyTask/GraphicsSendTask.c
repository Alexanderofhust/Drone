  ...
  while (1) 
	{
		...
		ShootLines_Init();
		YawUI_Change(IMUReceive.yawAngle,Init_Cnt);
		PitchUI_Change( IMUReceive.pitchAngle,Init_Cnt);//Pitch角度绘制
		GimbalMode_Change(droneState.ControlMode,Init_Cnt);//云台模式绘制
